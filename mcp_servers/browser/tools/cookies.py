    ...
    try:
        if session_manager.get_policy().get("mode") == "strict":
            raise SmartToolError(
                ...
                suggestion='Switch to permissive via browser(action="policy", mode="permissive") to mutate cookies',
            )
...
def get_all_cookies(
    ...
    with get_session(config) as (session, target):
        try:
            ...
            # Safety-as-mode: in strict policy, redact cookie values (keep metadata only).
            try:
                if session_manager.get_policy().get("mode") == "strict":
                    ...
                    for c in cookies:
                        ...
                        if "value" in c2:
                            val = c2.get("value")
                            try:
                                c2["valueLen"] = len(val) if isinstance(val, str) else None
                            except Exception:
                                c2["valueLen"] = None
                            c2["value"] = "<redacted>"
                            c2["redacted"] = True
    ...
    try:
        if session_manager.get_policy().get("mode") == "strict":
            raise SmartToolError(
                ...
                action="delete",
                reason="Blocked by strict policy",
                suggestion='Switch to permissive via browser(action="policy", mode="permissive") to delete cookies',
            )
    except SmartToolError:
        raise
    except Exception:
