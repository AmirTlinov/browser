...
def click_at_pixel(
    config: BrowserConfig, x: float, y: float, button: str = "left", click_count: int = 1
) -> dict[str, Any]:
    ...
    Args:
        ...
        click_count: Click count (default: 1)
    ...
    with get_session(config) as (session, target):
        try:
            session.click(x, y, button, click_count=max(1, int(click_count)))
            return {
                "x": x,
                "y": y,
                "button": button,
