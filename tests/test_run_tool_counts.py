...
def _install_hermetic_flow_mocks(monkeypatch: pytest.MonkeyPatch) -> None:
    """Hermetic mocks so run/flow tests don't require a real browser."""
...
def test_run_includes_tool_counts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    run_stats = res.data.get("run")
    assert isinstance(run_stats, dict)
    tool_counts = run_stats.get("toolCounts")
    assert isinstance(tool_counts, dict)
    assert tool_counts.get("click") == 1
