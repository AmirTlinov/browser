...
def test_include_memory_steps_recursion_is_blocked(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Self-referential runbook: rb -> include rb -> include rb -> ...
    session_manager.memory_set(
        key="rb",
        value=[{"macro": {"name": "include_memory_steps", "args": {"memory_key": "rb"}}}],
        max_bytes=20000,
        max_keys=200,
    )
    ...
    err = str(res.data.get("error") or "").lower()
    assert "recursive include_memory_steps" in err
